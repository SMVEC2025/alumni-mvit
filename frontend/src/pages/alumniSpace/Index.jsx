...
function AlumniSpace() {
  ...
  const isConfigMissing = !isSupabaseConfigured || !supabase
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      const fetchByUser = await sessionSupabase
        .from('alumni_registrations')
        .select('id, first_name, last_name, email, linkedin_url, phone, show_phone, degree, department, year_of_completion, address, city, state, country, pincode, work_experiences, company, designation, industry, experience, profile_image_url, cover_image_url, is_disabled')
        .eq('user_id', user.id)
        .maybeSingle()

      if (fetchByUser.error) {
        if (!mounted) return
        if (!hasCachedProfile) {
          setError(fetchByUser.error.message)
          setIsLoading(false)
        }
        return
      }

      const row = fetchByUser.data
      if (!row) {
        if (!hasCachedProfile) {
  ...
  const handleProfileImageChange = async (e) => {
    ...
    const sessionSupabase = getSupabaseWithSession()
    if (!registrationId || !sessionSupabase || !currentUserId) {
    ...
    try {
      ...
      const { error: updateError } = await sessionSupabase
        .from('alumni_registrations')
        .update({
          profile_image_url: persistedProfileImageUrl,
  ...
  const handleCoverImageChange = async (e) => {
    ...
    try {
      ...
      const { error: updateError } = await sessionSupabase
        .from('alumni_registrations')
        .update({ cover_image_url: persistedCoverUrl, updated_at: new Date().toISOString() })
        .eq('id', registrationId)
  ...
  const handleSubmit = async (e) => {
    ...
    const sessionSupabase = getSupabaseWithSession()
    if (!registrationId || !sessionSupabase) {
