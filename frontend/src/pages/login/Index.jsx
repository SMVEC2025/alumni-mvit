...
async function getRedirectPath(user) {
  ...
  return registered ? '/alumni-space' : '/register'
}
...
function Login() {
  ...
  const startOtpFlow = async (purpose, successMessage) => {
    ...
    // Move to OTP step immediately and send OTP in background to reduce perceived wait time.
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      setPasswordState('')
      setConfirmPassword('')
      setMessage(otpPurpose === 'forgot-password'
  ...
  const handleSkipPassword = async () => {
    ...
    setIsLoading(true)
    try {
      const result = await otpLogin(mobileNumber)
      const redirectPath = await getRedirectPath(result.user)
      navigate(redirectPath, { replace: true })
    } catch (err) {
