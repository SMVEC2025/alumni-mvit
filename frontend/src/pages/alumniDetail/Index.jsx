...
function AlumniDetail() {
    ...
    useEffect(() => {
        ...
        const fetchAlumniDetail = async () => {
            ...
            const { data, error: fetchError } = await sessionSupabase
            ...
            fetchSuggestedAlumni(data, viewerIsStaff)
        }

        const fetchSuggestedAlumni = async (currentAlumni, viewerIsStaff) => {
            const sessionSupabase = getSupabaseWithSession()
            if (!sessionSupabase) return

            let query = sessionSupabase
                .from('alumni_registrations')
                .select('id, first_name, last_name, department, designation, company, profile_image_url, degree, year_of_completion, is_disabled')
                .neq('id', id)
                .limit(20)
            ...
            // Prioritize same department, then random
            const sameDept = data.filter(a => a.department === currentAlumni.department)
            const otherDept = data.filter(a => a.department !== currentAlumni.department)

            // Shuffle arrays
            const shuffle = arr => arr.sort(() => Math.random() - 0.5)
            const suggested = [...shuffle(sameDept), ...shuffle(otherDept)].slice(0, 5)
    ...
    const handleCoverUpload = async (e) => {
        ...
        if (file.size > MAX_COVER_IMAGE_SIZE) {
            alert('Cover image must be 3MB or less.')
            return
        }
        ...
        try {
            ...
            setCoverUploading(false)
            alert('Failed to upload cover photo: ' + (uploadError?.message || 'Upload failed.'))
            return
        }

        if (!coverUrl) {
            setCoverUploading(false)
            alert('Failed to upload cover photo: Missing cover image URL.')
            return
        }
        ...
        if (updateError) {
            alert('Failed to update cover photo: ' + updateError.message)
            return
        }
    ...
    const handleToggleProfileStatus = async () => {
        ...
        const sessionSupabase = getSupabaseWithSession()
        if (!sessionSupabase) {
            setStatusError('Session not available. Please login again.')
            return
        }
        ...
        if (!updatedRow) {
            setStatusError('Update blocked. Apply the latest Supabase RLS SQL for staff updates.')
            return
        }
